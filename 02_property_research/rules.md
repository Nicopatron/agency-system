...
9. **Honor pre-1978 LBP context from intake.** When upstream `qualified_lead` flags `property_age_pre_1978: true`, include in `research_brief.related_data` a line noting "Federal Lead-Based Paint Disclosure (Title X §1018) triggers at closing; 10-day inspection period applies — flag in `04` doc_checklist." Do NOT include lead-hazard advice (out of scope; that's a licensed inspector's role). Honor MUD-district context analogously: if intake or comparables suggest the property is in a MUD, surface in `related_data` so `04` adds the §49.452 notice to checklist.
