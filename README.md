...
&nbsp;&nbsp;&nbsp;&nbsp;Both pages are deployed and live at the URLs above — GitHub Pages, free, public. The HTML files in this repo are the deployment artifacts; no infra-as-code config because none is needed.
...
**The runtime is a folder, not a UI.** Persistent state for every deal lives in `workflows/[client-YYYY-MM-DD]/`. Three files per case: `status.md` (current stage + active specialist + next action), `audit_log.md` (append-only chronological event log), `action_register.md` (open tasks). Any agent reads `status.md` to pick up where the last one left off; every state-changing route appends to `audit_log.md`. See [`workflows/Henderson-2026-04-28/audit_log.md`](./workflows/Henderson-2026-04-28/audit_log.md) for 21 real chronological events through option period + competing-offer escalation. This is the runtime — the same thing JSON-event-log systems give you, in markdown anyone can audit. Surface-agnostic: agents access it through Claude Projects today; an MCP server, Slack bridge, or CLI wraps the same `workflows/` reads tomorrow — the contracts don't change.
...
**Guided vs operational mode.** [`_config/team.md`](./_config/team.md) carries a `mode` per agent: `operational` (skip orchestrator, full authority on routing) or `guided` (route through 00_orchestrator, use `signing_agent_fallback` for comms, second-pair review for first 90 days). New agents ship in `guided` until their voice profile is built and the 90-day pair window closes. Diana flips the bit in `team.md` when ready — one column edit, no system rewiring.
...
10. **Defining the bar and enforcing it are two different jobs.** `_config/team-standards.md` defines Diana's quality bar. `05_quality_review` enforces it — checking every `comm_draft` against four criteria (specificity, clarity, brevity, voice) before the agent sees it. If a draft fails, revision notes go back to `03_client_communication`; after two cycles the specialist escalates to Diana. Escalations are logged in `escalation-log.md` and periodically reviewed to expand the playbook — so situations handled by escalation once get codified so the next one is handled by the system. The three confirmation gates (intake, hard compliance, quality) are enumerated as a single auditable index in [`_config/confirmation-gates.md`](./_config/confirmation-gates.md) — what a reviewer reads in 30 seconds to confirm coverage across all three failure categories.
...
13. **Known unknowns are first-class data.** Every handoff schema carries a `gaps: [string]` array — items the upstream specialist KNOWS it doesn't know about the case. Distinct from `verification_required` (an assertion needing second-source confirmation) and from `confidence` (probabilistic certainty about the output as a whole): `gaps` names specific holes that downstream work or the agent should capture during normal operation. Example on a 4/5 intake `qualified_lead`: `gaps: ["must_haves not fully captured", "deal_breakers not asked", "buyer_rep_agreement signed? (required pre-tour per SB 1968)"]`. Empty array (`gaps: []`) is explicit, never omitted — "nothing unknown left implicit" is itself a claim. Adapted from sparkles-inc's `agency-os` Handoff Card "GAPS" section (comp-4 peer submission); our YAML adaptation propagates the same concept through the schema-based handoff system. The cross-comp lesson: anti-typed-schema arguments work when the schema can't acknowledge unknowns. Naming the unknowns as a typed field defeats that argument.
14. **Texas 2026 regulatory anchors are wired into intake + closing, not described in prose.** The system tracks statutory + federal anchors that apply to Austin RE deals as concrete data fields, not as background context: `01_lead_qualifier` captures `property_age_pre_1978` (Federal LBP §1018 trigger), `buyer_rep_agreement_signed` (SB 1968 Jan 2026 pre-tour requirement), `first_time_buyer` (TDHCA/MFTH DPA eligibility), `mud_disclosure_required` (Texas Water Code §49.452 trigger). `04_transaction_coordinator` adds the corresponding docs to `doc_checklist` when triggers fire and flags rescission risk in `risks[]` when notices are missed. Each anchor independently verified against authoritative source (TREC, EPA, Texas Property Code, TDHCA) — audit trail in `../VERIFIED.md` (orchestration artifact, one level up). Re-verify annually as forms revise.
15. **Listings are a workflow type, not a separate specialist.** Listing-side cases flow through `workflows/[client-YYYY-MM-DD]/` with `workflow_type: seller_deal` — same `status.md`/`audit_log.md`/`action_register.md` triplet, same slip colors, same write protocol as buyer-side deals. The listing lifecycle (intake → MLS prep → goes-pending → closing) maps onto the same six-stage state machine; seller representation routes through `04_transaction_coordinator` for executed listings and through `03_client_communication` (seller-archetype calibration) for outbound. We don't ship a separate `08_listing_manager` specialist because the lifecycle is the same shape as a buyer deal — only the archetype calibration changes. See [`workflows/README.md`](./workflows/README.md) § Workflow types.
